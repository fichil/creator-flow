...
def test_list_provider_token_lifecycle_boundaries_returns_stable_metadata(
    client: TestClient,
):
    response = client.get("/api/provider-token-lifecycle-boundaries")

    assert response.status_code == 200
    body = response.json()
    assert list(body.keys()) == ["token_lifecycle_boundaries"]
    assert [
        boundary["provider_id"] for boundary in body["token_lifecycle_boundaries"]
    ] == [
        "fake_local",
        "douyin_sandbox",
        "douyin_real",
...
def test_fake_local_token_lifecycle_boundary_is_not_required_and_non_sensitive(
    client: TestClient,
):
    ...
    assert boundary["token_lifecycle_policy_status"] == "not_required"
    assert boundary["token_storage_policy_status"] == "none"
    assert boundary["refresh_policy_status"] == "not_required"
    assert boundary["expiry_policy_status"] == "not_required"
    assert boundary["revoke_policy_status"] == "not_required"
    assert boundary["disconnect_policy_status"] == "not_required"
    assert boundary["rotation_policy_status"] == "not_required"
    ...
    assert boundary["can_disconnect"] is False
    assert boundary["can_rotate_token"] is False
    assert boundary["can_mark_token_expired"] is False
    ...
    assert "no refresh token stored" in status_text
    assert "no token refresh" in status_text
    assert "no token revoke" in status_text
    assert "no disconnect operation" in status_text
    assert "no external service call" in status_text


def test_douyin_sandbox_token_lifecycle_boundary_is_placeholder_only(
    client: TestClient,
):
    ...
    assert boundary["token_lifecycle_policy_status"] == "not_implemented"
    assert boundary["token_storage_policy_status"] == "not_implemented"
    assert boundary["refresh_policy_status"] in {"required_planned", "not_implemented"}
    assert boundary["expiry_policy_status"] in {"required_planned", "not_implemented"}
    assert boundary["revoke_policy_status"] in {"required_planned", "not_implemented"}
    assert boundary["disconnect_policy_status"] in {
        "required_planned",
        "not_implemented",
    }
    assert boundary["rotation_policy_status"] in {
        "required_planned",
        "not_implemented",
    }
    ...
    assert boundary["can_mark_token_expired"] is False
    status_text = build_status_text(boundary)
    assert "placeholder token lifecycle boundary metadata only" in status_text
    assert "OAuth is not implemented" in status_text
    assert "tokens are not stored" in status_text
    assert "refresh tokens are not stored" in status_text
    assert "token refresh is not implemented" in status_text
    assert "token expiry handling is planned but not active" in status_text
    assert "token revoke is not implemented" in status_text
    assert "disconnect is not implemented" in status_text
...
def test_douyin_real_token_lifecycle_boundary_is_future_placeholder_only(
    client: TestClient,
):
    ...
    assert boundary["can_mark_token_expired"] is False
    status_text = build_status_text(boundary)
    assert "future real provider token lifecycle boundary placeholder only" in status_text
...
def test_get_each_provider_token_lifecycle_boundary_by_id(client: TestClient):
    for provider_id in ("fake_local", "douyin_sandbox", "douyin_real"):
        response = client.get(f"/api/provider-token-lifecycle-boundaries/{provider_id}")
...
def test_unknown_provider_token_lifecycle_boundary_returns_404_without_sensitive_terms(
    client: TestClient,
):
    response = client.get("/api/provider-token-lifecycle-boundaries/unknown")

    assert response.status_code == 404
    assert response.json()["detail"] == "Provider lifecycle boundary metadata not found"
    response_text = response.text.lower()
    for term in SENSITIVE_ERROR_TERMS:
        assert term not in response_text


def test_provider_token_lifecycle_response_does_not_leak_environment_values(
...
def test_provider_token_lifecycle_boundaries_table_exists_without_sensitive_columns(
    client: TestClient,
):
    with sqlite3.connect(get_settings().database_path) as connection:
        columns = connection.execute(
            "PRAGMA table_info(provider_token_lifecycle_boundaries)"
        ).fetchall()
    ...
    assert {
        "provider_id",
        "source_type",
        "token_lifecycle_policy_status",
        "token_storage_policy_status",
        "refresh_policy_status",
        "expiry_policy_status",
        "revoke_policy_status",
...
def test_unknown_provider_rows_are_not_returned_from_token_lifecycle_list(
    client: TestClient,
):
    with sqlite3.connect(get_settings().database_path) as connection:
        connection.execute(
            """
            INSERT INTO provider_token_lifecycle_boundaries (
                provider_id, source_type, token_lifecycle_policy_status,
    ...
    provider_ids = [
        ...
        for boundary in response.json()["token_lifecycle_boundaries"]
    ]
...
def test_known_provider_row_merges_non_sensitive_boundary_without_changing_registry(
    ...
    with sqlite3.connect(get_settings().database_path) as connection:
        connection.execute(
            ...
            (
                ...
                "planned",
                "planned",
                "Non-sensitive local fake lifecycle boundary override",
                "manual_test_metadata",
            ),
        )

    response = client.get("/api/provider-token-lifecycle-boundaries/fake_local")
    ...
    assert boundary["source_type"] == "fake_local"
    assert boundary["is_real_provider"] is False
    assert boundary["token_lifecycle_policy_status"] == "planned"
    assert boundary["token_storage_policy_status"] == "planned"
    assert boundary["refresh_policy_status"] == "required_planned"
    assert boundary["expiry_policy_status"] == "required_planned"
    assert boundary["revoke_policy_status"] == "required_planned"
    assert boundary["disconnect_policy_status"] == "required_planned"
    assert boundary["rotation_policy_status"] == "required_planned"
...
def test_provider_token_lifecycle_boundary_api_does_not_enable_write_methods(
    client: TestClient,
):
    write_responses = [
        client.post("/api/provider-token-lifecycle-boundaries", json={}),
        client.put("/api/provider-token-lifecycle-boundaries/fake_local", json={}),
        client.patch("/api/provider-token-lifecycle-boundaries/fake_local", json={}),
        client.delete("/api/provider-token-lifecycle-boundaries/fake_local"),
    ]
...
def test_existing_provider_metadata_routes_still_return_existing_metadata(
    ...
    oauth_boundaries_response = client.get("/api/provider-oauth-boundaries")
    ...
    assert oauth_boundaries_response.status_code == 200
    assert [
    ...
    assert [
        boundary["provider_id"]
        for boundary in oauth_boundaries_response.json()["oauth_boundaries"]
    ] == ["fake_local", "douyin_sandbox", "douyin_real"]
