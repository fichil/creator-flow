...
v1.0 is now in Batch 8 publish status reconciliation foundation work. v1.5 and v2.0 remain future roadmap targets, and the Batch 8 boundary does not make the current app production, commercial, or SaaS ready.
...
- v1.0 Batch 8 adds only a local publish status reconciliation and status snapshot foundation on top of the Batch 7 publish attempt workflow, Batch 6 publish intent workflow, and Batch 5 real provider controls boundary; it does not enable real provider runtime behavior, real OAuth runtime behavior, OAuth routes, real token exchange, real token storage, real credential storage, real provider calls, real status queries, upload, real publishing, scheduled publishing, metrics reads, or provider / upload / publish / status / external / Douyin response persistence.
...
| v1.0 | Douyin Integration User Test Release | Batch 8 publish status reconciliation foundation started; small user test only |
