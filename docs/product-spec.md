...
- 后续 callback route 必须依赖 Batch 2 OAuth state storage / anti-replay foundation，且仍需后续单独 route 批次批准；token exchange 必须等待 Batch 3；credential storage 必须等待 Batch 4；真实 OAuth runtime enablement 必须等待 Batch 5 feature flag / kill switch。
...
v0.7.0 已完成 local fake/manual metrics review summary workflow。v0.8.0 已完成 Provider & Credential Security Foundation release。v0.9.0 已发布为 Douyin Provider POC / Sandbox Integration。v1.0 当前进入 Batch 2 OAuth state storage / anti-replay foundation 阶段，用于在 Batch 1 callback contract 之后建立 internal-only digest-only state foundation；真实 OAuth runtime、OAuth URL、OAuth callback route、token lifecycle、credential storage、real provider API、real publish 和 real metrics 尚未实现。v1.0 的目标是进行用户抖音接入测试，不是生产级自动化发布版本，也不承诺批量发布、定时发布、多账号矩阵运营或自动内容优化。
...
本节描述 future roadmap，不是当前能力声明。当前已发布版本 v0.9.0 只有 sandbox/mock workflow、registry / factory foundation、sandbox-only backend API contract、frontend sandbox POC panel 和 roadmap / checklist 文档；v1.0 当前仅进入 Batch 2 OAuth state storage / anti-replay foundation，不提供真实抖音连接、真实 OAuth runtime、OAuth callback route、真实发布、生产级部署或多租户 SaaS。
