...
- 本批新增 frontend Douyin Sandbox POC Panel，是 v0.9 POC visual smoke surface。
- 用户可以看到 provider descriptors、sandbox mock connection result、sandbox metrics preview result 和 sandbox publish dry-run result。
- 所有按钮都只调用 Batch 6 sandbox-only backend API，所有展示结果都必须标注 deterministic sandbox / simulated / dry-run。
- `douyin_sandbox` 只能展示 sandbox-only POC result，不能展示为真实抖音账号连接、真实指标、真实上传、真实发布或真实排期。
- `douyin_real` 继续展示为 blocked / not implemented。
- unknown provider 不 fallback。
- 本批不会给用户提供真实 OAuth login、真实 connect、OAuth URL、token viewer、token / secret / credential 输入框、真实文件上传、真实视频上传、真实发布或真实排期入口。
- 本批不是客户商用功能，不是生产 UI，不表示可以真实发布抖音视频，也不表示已接入真实抖音开放平台。
- 本批不接真实 Douyin API，不实现 OAuth，不创建 OAuth URL，不新增 OAuth callback route，不新增 OAuth state storage，不交换或保存 token，也不读取环境变量密钥。
- 本批不访问真实平台，不抓取真实指标，不上传、不发布、不排期发布，不写真实 publication record，也不新增真实 tenant、billing、RBAC 或 admin console 实现。
- 本批不表示 v0.9 POC 已完成，不表示 v1.0、v1.5 或 v2.0 已完成。
...
- v0.9.0 只提供 sandbox-only deterministic POC 能力和 release review documentation。
- v0.9.0 不等于真实 Douyin integration，不等于 OAuth implementation，不等于 token storage，不等于真实指标读取，不等于真实发布，也不等于生产商用能力。

v1.0 Batch 1 产品边界：

- Batch 0 已完成 Douyin Integration User Test Release planning；Batch 1 只定义 OAuth boundary / callback contract。
- Batch 1 对应 ADR 0046：[`decisions/0046-v1.0-oauth-boundary-callback-contract.md`](decisions/0046-v1.0-oauth-boundary-callback-contract.md)，callback contract：[`contracts/v1.0-douyin-oauth-callback-contract.md`](contracts/v1.0-douyin-oauth-callback-contract.md)。
- 本批只定义未来真实 Douyin OAuth authorization start、success callback、provider error callback、unsupported provider、cancelled authorization、malformed callback、missing state、replayed callback 和 expired callback 的 contract 边界。
- 本批把未来安全响应类别限制为 `pending_token_exchange`、`authorization_cancelled`、`authorization_failed`、`invalid_callback`、`unsupported_provider` 和 `oauth_temporarily_disabled` 等安全状态。
- 本批不提供用户可操作的真实 OAuth login、真实 connect、OAuth URL、OAuth callback、token viewer、credential 管理、真实指标、上传、发布或排期发布入口。
- 本批不修改数据库表，不新增 backend API，不新增 frontend UI，不新增真实 Provider，不读取环境变量密钥，不调用 Douyin API 或外部服务。
- 后续 callback route 必须依赖 Batch 2 OAuth state storage / anti-replay foundation，且仍需后续单独 route 批次批准；Batch 3 只允许建立 internal-only / fake-gated token exchange boundary，不代表真实 token exchange；credential storage 必须等待 Batch 4；真实 OAuth runtime enablement 必须等待 Batch 5 feature flag / kill switch。
- v1.0 不是生产商用版本，不承诺 SLA，不承诺多租户 SaaS，不承诺客户的客户商用，不承诺批量自动发布，也不绕过平台审核、权限或用户授权。
- v1.0 不等于 v1.5 Minimum Production Release，也不等于 v2.0 Multi-Tenant SaaS Commercial Release。

v1.0 Batch 2 产品边界：

- Batch 2 只建立 OAuth state storage / anti-replay foundation。
- Batch 2 对应 ADR 0047：[`decisions/0047-v1.0-oauth-state-storage-anti-replay.md`](decisions/0047-v1.0-oauth-state-storage-anti-replay.md)，state storage contract：[`contracts/v1.0-douyin-oauth-state-storage-contract.md`](contracts/v1.0-douyin-oauth-state-storage-contract.md)。
- 本批新增 internal-only backend state foundation 和 `provider_oauth_states` digest-only schema。
- 本批只允许保存 state digest 和 provider、source、status、created / expires / consumed metadata、safe status message 等安全元数据，不保存 raw OAuth state 或 OAuth state value。
- 本批提供 one-time consume、replay reject、expired reject、provider mismatch reject、unsupported provider reject、source separation 和 redaction 测试基础。
- 本批不提供用户可操作的真实 OAuth login、真实 connect、OAuth URL、OAuth start route、OAuth callback route、token viewer、credential 管理、真实指标、上传、发布或排期发布入口。
- 本批不做 token exchange，不保存 token / secret / credential / authorization code，不新增 frontend OAuth UI，不启用 `douyin_real`，不允许 `douyin_real` fallback 到 `douyin_sandbox`。
- v1.0 不是生产商用版本，不承诺 SLA，不承诺多租户 SaaS，不承诺客户的客户商用，不承诺批量自动发布，也不绕过平台审核、权限或用户授权。
- v1.0 不等于 v1.5 Minimum Production Release，也不等于 v2.0 Multi-Tenant SaaS Commercial Release。

v1.0 Batch 3 产品边界：

- Batch 3 只建立 internal-only token exchange boundary with fake-gated integration。
- Batch 3 对应 ADR 0048：[`decisions/0048-v1.0-token-exchange-boundary-fake-gated.md`](decisions/0048-v1.0-token-exchange-boundary-fake-gated.md)，token exchange boundary contract：[`contracts/v1.0-douyin-token-exchange-boundary-contract.md`](contracts/v1.0-douyin-token-exchange-boundary-contract.md)，test matrix：[`testing/v1.0-token-exchange-boundary-test-matrix.md`](testing/v1.0-token-exchange-boundary-test-matrix.md)。
- 本批新增 internal-only backend token exchange boundary service 和 fake-gated simulator 测试，用于验证 Batch 2 state foundation 依赖、one-time consume、replay / expired / provider mismatch rejection、real provider disabled 和 sandbox fallback forbidden。
- fake-gated exchange 只能返回 metadata-only / redacted result，例如 provider、source、安全状态、是否收到模拟 token 响应、是否需要后续 credential storage；不得返回、保存、记录 authorization code、raw OAuth state、access token、refresh token、token、secret、credential、cookie、session、raw request 或 raw response。
- 本批不新增 DB schema，不新增 OAuth URL，不新增 OAuth start route，不新增 OAuth callback route，不新增真实 token exchange，不新增 token storage，不新增 credential storage，不新增 frontend OAuth UI，不调用 Douyin API 或任何业务外部服务。
- `douyin_real` 继续默认 disabled / blocked，且不允许 fallback 到 `douyin_sandbox`。
- v1.0 不是生产商用版本，不承诺 SLA，不承诺多租户 SaaS，不承诺客户的客户商用，不承诺批量自动发布，也不绕过平台审核、权限或用户授权。
- v1.0 不等于 v1.5 Minimum Production Release，也不等于 v2.0 Multi-Tenant SaaS Commercial Release。

v1.0 Batch 4 产品边界：

- Batch 4 只建立 internal-only credential reference / encrypted storage boundary design。
- Batch 4 对应 ADR 0049：[`decisions/0049-v1.0-credential-reference-encrypted-storage-design.md`](decisions/0049-v1.0-credential-reference-encrypted-storage-design.md)，credential storage boundary contract：[`contracts/v1.0-douyin-credential-storage-boundary-contract.md`](contracts/v1.0-douyin-credential-storage-boundary-contract.md)，test matrix：[`testing/v1.0-credential-storage-boundary-test-matrix.md`](testing/v1.0-credential-storage-boundary-test-matrix.md)。
- 本批新增 internal-only backend credential storage boundary service，用于验证 Batch 3 safe token exchange metadata、metadata-only credential reference、raw token / authorization code / OAuth state / secret material rejection、encryption provider unavailable、real provider disabled 和 sandbox fallback forbidden。
- 本批复用既有 `provider_credential_references` metadata-only schema，不新增 encrypted payload storage，不保存 authorization code、raw OAuth state、access token、refresh token、token、secret、credential、cookie、session、raw request 或 raw response。
- 本批不使用 base64、hash、xor、普通编码或自定义算法冒充 encryption；没有可靠加密依赖和密钥管理设计时，encrypted storage path 只能返回 safe unavailable / disabled category。
- 本批不新增 OAuth URL，不新增 OAuth start route，不新增 OAuth callback route，不新增真实 token exchange，不新增真实 token storage，不启用真实 credential storage，不新增 frontend OAuth UI，不调用 Douyin API 或任何业务外部服务。
- `douyin_real` 继续默认 disabled / blocked，且不允许 fallback 到 `douyin_sandbox`。
- v1.0 不是生产商用版本，不承诺 SLA，不承诺多租户 SaaS，不承诺客户的客户商用，不承诺批量自动发布，也不绕过平台审核、权限或用户授权。
- v1.0 不等于 v1.5 Minimum Production Release，也不等于 v2.0 Multi-Tenant SaaS Commercial Release。
...
- Batch 5 只建立 internal-only real provider feature flag / kill switch foundation。
- Batch 5 对应 ADR 0050：[`decisions/0050-v1.0-real-provider-feature-flag-kill-switch.md`](decisions/0050-v1.0-real-provider-feature-flag-kill-switch.md)，feature flag / kill switch contract：[`contracts/v1.0-real-provider-feature-flag-kill-switch-contract.md`](contracts/v1.0-real-provider-feature-flag-kill-switch-contract.md)，test matrix：[`testing/v1.0-real-provider-feature-flag-kill-switch-test-matrix.md`](testing/v1.0-real-provider-feature-flag-kill-switch-test-matrix.md)。
- 本批新增 internal-only backend real provider controls service，用于验证 known provider、source type、feature flag、kill switch、capability、environment、platform preconditions 和 sandbox fallback attempt。
- `douyin_real` 默认 disabled / blocked；kill switch active、missing、malformed、revoked、expired 或 policy denied 均必须安全阻断；missing / malformed feature flag、unsupported provider、unsupported capability 和 platform preconditions missing 均必须安全阻断。
- 本批不新增 DB schema，不启用真实 provider，不新增 OAuth URL，不新增 OAuth start route，不新增 OAuth callback route，不新增真实 token exchange，不新增真实 token storage，不启用真实 credential storage，不新增 frontend OAuth UI，不调用 Douyin API 或任何业务外部服务。
- 本批不保存 authorization code、raw OAuth state、access token、refresh token、token、secret、client secret、credential、API key、cookie、session、bearer、raw request 或 raw response。
...
- Batch 6 只建立本地 user-confirmed publish intent workflow foundation。
- Batch 6 对应 ADR 0051：[`decisions/0051-v1.0-user-confirmed-publish-intent-workflow.md`](decisions/0051-v1.0-user-confirmed-publish-intent-workflow.md)，publish intent contract：[`contracts/v1.0-user-confirmed-publish-intent-contract.md`](contracts/v1.0-user-confirmed-publish-intent-contract.md)，test matrix：[`testing/v1.0-user-confirmed-publish-intent-test-matrix.md`](testing/v1.0-user-confirmed-publish-intent-test-matrix.md)。
- 本批新增本地 publish intent workflow、metadata-only publish intent schema extension、local-only backend API、frontend Publish Intent UI 和测试，用于验证发布前必须经过用户显式确认、review state 关联、preflight validation 和 duplicate prevention。
- publish intent 只表示本地发布意图已记录，不表示上传、发布、排期发布或真实 Douyin 调用已经发生。
- 本批仍不启用真实 provider，不新增真实 publish adapter，不新增 OAuth URL，不新增 OAuth start route，不新增 OAuth callback route，不新增真实 token exchange，不新增真实 token storage，不启用真实 credential storage，不新增 frontend OAuth UI，不调用 Douyin API 或任何业务外部服务。
- 本批不保存 authorization code、raw OAuth state、access token、refresh token、token、secret、client secret、credential、API key、cookie、session、bearer、raw request、raw response、provider response、upload response 或 publish response。
- `douyin_real` 继续默认 disabled / blocked，且不允许 fallback 到 `douyin_sandbox`。
- v1.0 不是生产商用版本，不承诺 SLA，不承诺多租户 SaaS，不承诺客户的客户商用，不承诺批量自动发布，也不绕过平台审核、权限或用户授权。
- v1.0 不等于 v1.5 Minimum Production Release，也不等于 v2.0 Multi-Tenant SaaS Commercial Release。
...
- Batch 7 建立 guarded real publish adapter foundation，但默认不执行真实发布。
- Batch 7 对应 ADR 0052：[`decisions/0052-v1.0-real-publish-adapter-guarded-implementation.md`](decisions/0052-v1.0-real-publish-adapter-guarded-implementation.md)，guarded publish adapter contract：[`contracts/v1.0-real-publish-adapter-guarded-contract.md`](contracts/v1.0-real-publish-adapter-guarded-contract.md)，test matrix：[`testing/v1.0-real-publish-adapter-guarded-test-matrix.md`](testing/v1.0-real-publish-adapter-guarded-test-matrix.md)。
- 本批新增 metadata-only publish attempt schema、guarded adapter internal service、local-only guarded attempt API、frontend guarded attempt UI 和测试，用于验证真实发布前必须依赖 confirmed publish intent、Batch 5 controls、review/media/metadata/credential reference preflight 和 duplicate attempt prevention。
- publish attempt 只表示本地 guarded attempt 已记录，不表示上传、真实发布、排期发布、真实 Douyin 调用或真实 metrics read 已经发生。
- 本批仍不启用真实 provider，不创建 OAuth URL，不新增 OAuth start route，不新增 OAuth callback route，不做真实 token exchange，不保存真实 token / secret / credential / authorization code / raw OAuth state，不新增 frontend OAuth UI，不调用 Douyin API 或任何业务外部服务。
- 本批不保存 provider response、upload response、publish response、external response、Douyin response、raw request 或 raw response。
- `douyin_real` 继续默认 disabled / blocked，kill switch 和 platform preconditions 继续安全阻断，且不允许 fallback 到 `douyin_sandbox`。
- v1.0 不是生产商用版本，不承诺 SLA，不承诺多租户 SaaS，不承诺客户的客户商用，不承诺批量自动发布，也不绕过平台审核、权限或用户授权。
- v1.0 不等于 v1.5 Minimum Production Release，也不等于 v2.0 Multi-Tenant SaaS Commercial Release。

v1.0 Batch 8 产品边界：

- Batch 8 建立 publish status reconciliation foundation，但默认不执行真实 Douyin 状态查询。
- Batch 8 对应 ADR 0053：[`decisions/0053-v1.0-publish-status-reconciliation.md`](decisions/0053-v1.0-publish-status-reconciliation.md)，publish status reconciliation contract：[`contracts/v1.0-publish-status-reconciliation-contract.md`](contracts/v1.0-publish-status-reconciliation-contract.md)，test matrix：[`testing/v1.0-publish-status-reconciliation-test-matrix.md`](testing/v1.0-publish-status-reconciliation-test-matrix.md)。
- 本批新增 metadata-only publish status reconciliation schema、metadata-only status snapshot schema、internal reconciliation service、local-only API、frontend Local Status Reconciliation / Publish Status Snapshot UI 和测试，用于验证状态对账必须依赖 existing Batch 7 publish attempt。
- status reconciliation 和 status snapshot 只表示本地安全状态记录，不表示已经查询真实抖音、确认真实发布成功、读取真实平台状态或抓取真实指标。
- 本批仍不启用真实 provider，不创建 OAuth URL，不新增 OAuth start route，不新增 OAuth callback route，不做真实 token exchange，不保存真实 token / secret / credential / authorization code / raw OAuth state，不新增 frontend OAuth UI，不调用 Douyin API 或任何业务外部服务。
- 本批不保存 provider response、upload response、publish response、status response、external response、Douyin response、metrics response、raw request 或 raw response。
- `douyin_real` 继续默认 disabled / blocked，kill switch 和 platform preconditions 继续安全阻断，且不允许 fallback 到 `douyin_sandbox`。
- v1.0 不是生产商用版本，不承诺 SLA，不承诺多租户 SaaS，不承诺客户的客户商用，不承诺批量自动发布，也不绕过平台审核、权限或用户授权。
- v1.0 不等于 v1.5 Minimum Production Release，也不等于 v2.0 Multi-Tenant SaaS Commercial Release。
...
- Batch 9 建立 limited metrics read guarded foundation，但默认不执行真实 Douyin metrics query。
- Batch 9 对应 ADR 0054：[`decisions/0054-v1.0-limited-metrics-read-guarded-foundation.md`](decisions/0054-v1.0-limited-metrics-read-guarded-foundation.md)，limited metrics read guarded contract：[`contracts/v1.0-limited-metrics-read-guarded-contract.md`](contracts/v1.0-limited-metrics-read-guarded-contract.md)，test matrix：[`testing/v1.0-limited-metrics-read-guarded-test-matrix.md`](testing/v1.0-limited-metrics-read-guarded-test-matrix.md)。
- 本批新增 metadata-only metrics snapshot schema、internal metrics read guard service、local-only API、frontend Limited Metrics Snapshot UI 和测试，用于验证 metrics snapshot 必须依赖 existing Batch 8 publish status snapshot，并通过 Batch 7 publish attempt、Batch 6 publish intent 和 Batch 5 real provider controls 的链路。
- metrics snapshot 只表示本地 / fake / sandbox-safe 指标 metadata，不表示已经查询真实抖音、获取真实平台指标、验证真实运营表现或读取真实平台状态。
- 本批必须展示 freshness，例如 `fresh`、`stale`、`unknown` 或 `not_available`，并以安全文案展示 metrics permission / platform limitation。
- 本批仍不启用真实 provider，不创建 OAuth URL，不新增 OAuth start route，不新增 OAuth callback route，不做真实 token exchange，不保存真实 token / secret / credential / authorization code / raw OAuth state，不新增 frontend OAuth UI，不调用 Douyin API 或任何业务外部服务。
- 本批不保存 provider response、upload response、publish response、status response、metrics response、external response、Douyin response、raw request 或 raw response。
- `douyin_real` 继续默认 disabled / blocked，kill switch 和 platform preconditions 继续安全阻断，且不允许 fallback 到 `douyin_sandbox`。
- v1.0 不是生产商用版本，不承诺 SLA，不承诺多租户 SaaS，不承诺客户的客户商用，不承诺批量自动发布，也不绕过平台审核、权限或用户授权。
- v1.0 不等于 v1.5 Minimum Production Release，也不等于 v2.0 Multi-Tenant SaaS Commercial Release。
...
- Batch 10 建立 user test readiness / release candidate package，但不是正式 v1.0 release tag，也不创建或修改 `v1.0.0` tag。
- Batch 10 对应 ADR 0055：[`decisions/0055-v1.0-user-test-readiness-release-candidate.md`](decisions/0055-v1.0-user-test-readiness-release-candidate.md)，RC checklist：[`releases/v1.0-user-test-rc-checklist.md`](releases/v1.0-user-test-rc-checklist.md)，release notes draft：[`releases/v1.0-user-test-release-notes-draft.md`](releases/v1.0-user-test-release-notes-draft.md)，PR draft：[`releases/v1.0-pr-description-draft.md`](releases/v1.0-pr-description-draft.md)，user test guide：[`testing/v1.0-user-test-guide.md`](testing/v1.0-user-test-guide.md)，rollback / disablement runbook：[`operations/v1.0-user-test-rollback-disablement.md`](operations/v1.0-user-test-rollback-disablement.md)，RC test matrix：[`testing/v1.0-user-test-rc-test-matrix.md`](testing/v1.0-user-test-rc-test-matrix.md)。
- 本批只收口 Batch 0-9 的 local-only / metadata-only / guarded foundation 验收材料、风险边界、测试矩阵、安全扫描、禁用和回滚方案、用户测试脚本、release notes draft 和 PR draft。
- 本批不新增业务平台能力，不新增 DB schema，不新增 API route，不新增真实 provider runtime，不启用 `douyin_real`，不允许 `douyin_real` fallback 到 `douyin_sandbox`。
- 本批仍不创建 OAuth URL，不新增 OAuth start route，不新增 OAuth callback route，不做真实 token exchange，不保存真实 token / secret / credential / authorization code / raw OAuth state，不新增 frontend OAuth UI，不调用 Douyin API 或任何业务外部服务。
- 本批不上传、不真实发布、不排期发布、不查询真实 Douyin 状态、不抓取真实 Douyin 指标，不保存 provider response、upload response、publish response、status response、metrics response、external response、Douyin response、raw request 或 raw response。
- Batch 10 可以形成小范围用户测试前的 RC readiness package，但不声明 v1.0、v1.5 或 v2.0 已完成，也不声明 production、commercial 或 SaaS readiness。

真实 Douyin API、真实 OAuth、真实 OAuth callback route、真实 token exchange、真实 token storage、真实 credential storage、真实状态查询、真实指标读取、上传、发布、排期发布和自动发布都必须等后续单独 ADR、单独分支、单独测试和安全扫描通过后才能进入。

## Road to Douyin user testing

v0.7.0 已完成 local fake/manual metrics review summary workflow。v0.8.0 已完成 Provider & Credential Security Foundation release。v0.9.0 已发布为 Douyin Provider POC / Sandbox Integration。v1.0 当前进入 Batch 10 user test readiness / release candidate package 阶段，用于收口 Batch 0-9 的 local-only / metadata-only / guarded foundation 验收材料、风险边界、测试矩阵、release notes / PR draft、user test guide、rollback / disablement runbook 和验证脚本；真实 provider 仍未启用，真实 OAuth runtime、OAuth URL、OAuth callback route、真实 token exchange、真实 token storage、真实 credential storage、real provider API、真实 upload、真实 publish、scheduled publish、real status query 和 real metrics 尚未实现。v1.0 的目标是进行用户抖音接入测试，不是生产级自动化发布版本，也不承诺批量发布、定时发布、多账号矩阵运营或自动内容优化。
...
- 产品需要有撤销授权或断开连接的方向，即使后续 v1.0 用户测试只先覆盖所需的最小路径。
- 如果 Douyin API 权限不可用，v0.9 / v1.0 用户测试规划可以使用 manual import 或 sandbox/mock provider contract test 作为 fallback。
...
本节描述 future roadmap，不是当前能力声明。当前已发布版本 v0.9.0 只有 sandbox/mock workflow、registry / factory foundation、sandbox-only backend API contract、frontend sandbox POC panel 和 roadmap / checklist 文档；v1.0 当前仅进入 Batch 10 user test readiness / release candidate package，不创建正式 v1.0 tag 或 `v1.0.0` tag，不启用真实 provider，也不提供真实抖音连接、真实 OAuth runtime、OAuth callback route、真实 token exchange、真实 token storage、真实 credential storage、真实上传、真实发布、排期发布、真实状态查询、真实指标读取、生产级部署或多租户 SaaS。
