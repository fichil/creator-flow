...
本路线图描述计划和 readiness target，不代表当前仓库已经具备未来版本能力。当前已发布版本是 `v0.9.0 - Douyin Provider POC / Sandbox Integration`；v1.0 已进入 Batch 2 OAuth state storage / anti-replay foundation 阶段，但真实 Douyin OAuth runtime、token exchange、credential storage、publish、metrics 能力尚未开始实现。v1.5 和 v2.0 仍是后续目标，不是当前能力声明。
...
- v0.9.0 发布后，v1.0 已完成 docs-only / planning-only 的 Batch 0 和 docs-only / contract-only 的 Batch 1，并进入 Batch 2 OAuth state storage / anti-replay foundation；任一后续真实实现方向都必须继续保持无真实 Douyin / OAuth runtime / token / publish / metrics 的边界，直到单独 ADR、平台权限、应用审核、用户授权、测试和安全门槛批准真实能力。
...
状态：Batch 2 OAuth state storage / anti-replay foundation。当前已在 Batch 1 定义未来真实 Douyin OAuth authorization start、callback、error callback、unsupported provider、cancelled authorization、malformed callback、replayed callback 和 expired callback 的 contract 边界，并在 Batch 2 增加 internal-only digest-only OAuth state foundation；仍不实现真实 OAuth runtime、OAuth URL、OAuth start route、OAuth callback route、token exchange、token storage、credential storage、real provider API、publish workflow 或 metrics read。
...
- 明确后续 callback route 必须依赖 Batch 2 OAuth state storage / anti-replay foundation，且仍需后续单独 route 批次批准；token exchange 必须等待 Batch 3，credential storage 必须等待 Batch 4，runtime feature flag / kill switch enablement 必须等待 Batch 5。
