...
状态：Completed / Released。v0.9.0 已从 release commit `0f5263452b65077f2c70c82e506944dd46e60e96` 发布为 Douyin Provider POC / Sandbox Integration。v0.9 Batch 0 已完成 Douyin Provider POC / Sandbox Integration 的 planning、ADR 和 checklist；Batch 1 已完成 Douyin Provider Adapter Skeleton backend foundation；Batch 2 已完成 `douyin_sandbox` 的 sandbox-only operation simulation；Batch 3 已完成 backend-only Douyin Provider Registry / Factory Routing foundation；Batch 4 已完成 backend-only Douyin Provider Sandbox Metrics / Mock Workflow POC；Batch 5 已完成 docs-only / planning-only 的 roadmap to v2.0 commercial release alignment；Batch 6 已完成 backend-only Douyin Sandbox API Contract / Smoke Endpoints；Batch 7 已完成 frontend Douyin Sandbox POC Panel；Batch 8 已完成 POC readiness finalization / release candidate package；Batch 9 已完成 Release Branch / PR Merge Preparation package。v0.9.0 仍不是商用版本，不是真实 provider integration，不调用真实 Douyin API，不实现 OAuth，不创建 OAuth URL，不新增 OAuth callback route，不交换或保存 token，不抓取真实指标，不上传、不发布、不排期发布。
...
- Douyin Provider POC Readiness Finalization / Release Candidate package。
- 新增 v0.9 RC checklist / release candidate 文档。
- 新增 v0.9 test matrix 文档。
- 新增本地 validation script，用于运行 backend tests、frontend tests、frontend build、`git diff --check`、smoke-api 和关键词扫描。
- 更新 README、roadmap、architecture、product spec、development docs、v0.9 readiness checklist、v1-to-v2 roadmap 和 ADR。
- 汇总 human review / PR / merge / release decision 所需的验证边界。
...
- v0.9.0 发布后，v1.0 已完成 docs-only / planning-only 的 Batch 0、docs-only / contract-only 的 Batch 1、Batch 2 OAuth state storage / anti-replay foundation、Batch 3 token exchange boundary / fake-gated integration、Batch 4 credential reference / encrypted storage design、Batch 5 real provider feature flag / kill switch、Batch 6 user-confirmed publish intent workflow、Batch 7 guarded real publish adapter foundation、Batch 8 publish status reconciliation foundation、Batch 9 limited metrics read guarded foundation，并进入 Batch 10 user test readiness / release candidate package；任一后续真实实现方向都必须继续保持无真实 Douyin / OAuth runtime / real token exchange / real token storage / real credential storage / real upload / real publish / scheduled publish / real status query / real metrics read 的边界，直到单独 ADR、平台权限、应用审核、用户授权、测试和安全门槛批准真实能力。

## v1.0 Douyin Integration User Test Release

目标：准备可以进行小范围用户抖音接入测试的 v1.0 release candidate 证据包，验证用户授权、账号连接状态和未来真实或 sandbox/manual fallback 指标回流路径的可行性边界。Batch 10 不创建正式 v1.0 tag 或 `v1.0.0` tag；v1.0 目标仍是 User Test Release，不是生产级自动化运营或自动发布版本。

状态：Batch 10 user test readiness / release candidate package。当前已在 Batch 1 定义未来 OAuth callback contract，在 Batch 2 增加 internal-only digest-only OAuth state foundation，在 Batch 3 增加 internal-only token exchange boundary / fake-gated simulator，在 Batch 4 增加 internal-only credential storage boundary，在 Batch 5 增加 internal-only real provider controls，在 Batch 6 增加本地 publish intent workflow、显式确认、preflight validation 和 duplicate prevention，在 Batch 7 增加 metadata-only guarded publish attempt、guarded adapter boundary、preflight guard 和 duplicate attempt prevention，在 Batch 8 增加 metadata-only publish status reconciliation、status snapshot、local-only reconciliation API/UI、duplicate reconciliation prevention 和 stale status ignored，在 Batch 9 增加 metadata-only metrics snapshot、local-only metrics API/UI、freshness visibility、metrics permission / platform limitation safe message，并在 Batch 10 增加 RC checklist、release notes draft、PR draft、user test guide、rollback / disablement runbook、RC test matrix 和验证脚本；仍不启用真实 provider，也不实现真实 OAuth runtime、OAuth URL、OAuth start route、OAuth callback route、真实 token exchange、真实 token storage、真实 credential storage、real provider API、真实上传、真实发布、排期发布、真实 Douyin 状态查询或真实指标读取。

v1.0 Batch 0 允许范围：

- 新增 v1.0 Douyin Integration User Test Release planning ADR。
- 新增 v1.0 Douyin Integration User Test Release plan。
- 更新 README、README.en、roadmap、architecture、product spec、development docs、v1.0 readiness checklist 和 v1-to-v2 roadmap。
- 规划真实 Douyin OAuth、token lifecycle、credential storage、publish workflow、metrics read、audit、安全边界、kill switch 和用户测试验收条件。
- 拆分后续 Batch 1-N 的开发路径。
- 明确平台权限、应用审核、callback URL、OAuth scope、用户授权和安全存储设计是后续真实能力的前置条件。
...
- 明确后续 callback route 必须依赖 Batch 2 OAuth state storage / anti-replay foundation，且仍需后续单独 route 批次批准；Batch 3 只建立 internal-only / fake-gated token exchange boundary，真实 token exchange 仍需后续单独实现和安全验收；credential storage 必须等待 Batch 4，runtime feature flag / kill switch enablement 必须等待 Batch 5。
...
- 新增 v1.0 OAuth state storage / anti-replay ADR：[`decisions/0047-v1.0-oauth-state-storage-anti-replay.md`](decisions/0047-v1.0-oauth-state-storage-anti-replay.md)。
- 新增 internal-only OAuth state storage contract：[`contracts/v1.0-douyin-oauth-state-storage-contract.md`](contracts/v1.0-douyin-oauth-state-storage-contract.md)。
- 新增 state storage test matrix：[`testing/v1.0-oauth-state-storage-test-matrix.md`](testing/v1.0-oauth-state-storage-test-matrix.md)。
- 新增 `provider_oauth_states` digest-only schema，用于保存 provider、source、state digest、status、created / expires / consumed metadata 和安全状态说明。
- 新增 backend-only internal OAuth state foundation service，用于生成一次性 raw state、只持久化 digest、校验 provider、一次性 consume、拒绝 replay、拒绝 expired state、拒绝 malformed / missing state、拒绝 provider mismatch 和 unsupported provider。
- 新增 anti-replay、expiry、provider isolation、redaction、no external service 和 no OAuth route 测试。
...
- 不新增 OAuth callback route。
- 不做 token exchange。
- 不保存 token、secret、API key、credential、authorization code、raw OAuth state 或 OAuth state value。
- 不新增 frontend OAuth UI。
...
- 新增 v1.0 token exchange boundary / fake-gated integration ADR：[`decisions/0048-v1.0-token-exchange-boundary-fake-gated.md`](decisions/0048-v1.0-token-exchange-boundary-fake-gated.md)。
- 新增 internal-only token exchange boundary contract：[`contracts/v1.0-douyin-token-exchange-boundary-contract.md`](contracts/v1.0-douyin-token-exchange-boundary-contract.md)。
- 新增 token exchange boundary test matrix：[`testing/v1.0-token-exchange-boundary-test-matrix.md`](testing/v1.0-token-exchange-boundary-test-matrix.md)。
- 新增 backend-only internal token exchange boundary service，用于校验 provider、authorization code fake gate、Batch 2 state consumption、replay / expired / provider mismatch / unsupported provider 拒绝、`douyin_real` 默认 disabled 和 sandbox fallback forbidden。
- 新增 fake-gated exchange simulator，只返回 metadata-only / redacted result，不返回、不保存、不记录任何 token material。
- 新增 state dependency、no token material、no external service、no OAuth route、no token storage、no credential storage 测试。
...
- 不保存 authorization code、access token、refresh token、token、secret、credential、cookie、session、raw OAuth state、raw request 或 raw response。
- 不新增 DB schema。
- 不新增 credential storage。
- 不新增 frontend OAuth UI。
- 不新增真实 Provider runtime behavior。
...
- 新增 v1.0 credential reference / encrypted storage design ADR：[`decisions/0049-v1.0-credential-reference-encrypted-storage-design.md`](decisions/0049-v1.0-credential-reference-encrypted-storage-design.md)。
- 新增 internal-only credential storage boundary contract：[`contracts/v1.0-douyin-credential-storage-boundary-contract.md`](contracts/v1.0-douyin-credential-storage-boundary-contract.md)。
- 新增 credential storage boundary test matrix：[`testing/v1.0-credential-storage-boundary-test-matrix.md`](testing/v1.0-credential-storage-boundary-test-matrix.md)。
- 新增 backend-only internal credential storage boundary service，用于校验 provider、Batch 3 safe metadata result、raw material rejection、encryption provider unavailable、`douyin_real` 默认 disabled 和 sandbox fallback forbidden。
- 复用既有 `provider_credential_references` metadata-only schema 创建 credential reference metadata；不新增 encrypted payload storage。
- 新增 metadata-only reference、raw token / authorization code / OAuth state / secret material rejection、no external service、no OAuth route、no real token storage 和 no real credential storage 测试。
...
- 不实现真实 token exchange。
- 不保存 authorization code、access token、refresh token、token、secret、credential、cookie、session、raw OAuth state、raw request 或 raw response。
- 不新增 encrypted payload storage。
- 不启用真实 token storage。
- 不启用真实 credential storage。
- 不使用 base64、hash、xor、普通编码或自定义算法冒充 encryption。
- 不新增 frontend OAuth UI。
- 不新增真实 Provider runtime behavior。
- 不启用 `douyin_real`。
- 不允许 `douyin_real` fallback 到 `douyin_sandbox`。
...
- 新增 v1.0 user-confirmed publish intent workflow ADR：[`decisions/0051-v1.0-user-confirmed-publish-intent-workflow.md`](decisions/0051-v1.0-user-confirmed-publish-intent-workflow.md)。
- 新增 user-confirmed publish intent contract：[`contracts/v1.0-user-confirmed-publish-intent-contract.md`](contracts/v1.0-user-confirmed-publish-intent-contract.md)。
- 新增 user-confirmed publish intent test matrix：[`testing/v1.0-user-confirmed-publish-intent-test-matrix.md`](testing/v1.0-user-confirmed-publish-intent-test-matrix.md)。
- 新增本地 publish intent workflow，用于证明发布前必须由用户显式确认，并且必须关联同一项目内 approved / ready review item。
- 扩展 `publish_intents` metadata-only schema，记录 provider、source、confirmation status、安全状态说明和本地状态变更原因；不保存 provider response、upload response、publish response 或任何 credential material。
- 新增 local-only backend API 和 frontend Publish Intent UI，用于创建、查看和取消本地发布意图；UI 必须说明这是 Publish Intent，不是已经上传、发布或排期。
- 新增 preflight validation、media readiness、metadata validation、duplicate active intent prevention、`douyin_real` disabled、sandbox fallback forbidden、no external service、no real publish 和 no metrics tests。

v1.0 Batch 6 明确不是：

- 不新增真实 publish adapter。
- 不调用 Douyin API 或业务外部服务。
- 不上传视频。
- 不发布视频。
- 不排期发布。
...
- 不实现真实 OAuth。
- 不实现真实 token exchange。
- 不保存 authorization code、access token、refresh token、token、secret、credential、cookie、session、API key、bearer、raw OAuth state、raw request、raw response、provider response、upload response 或 publish response。
- 不新增真实 token storage。
- 不新增真实 credential storage。
- 不新增 frontend OAuth UI。
- 不抓取真实指标。
...
- 新增 v1.0 guarded real publish adapter ADR：[`decisions/0052-v1.0-real-publish-adapter-guarded-implementation.md`](decisions/0052-v1.0-real-publish-adapter-guarded-implementation.md)。
- 新增 guarded real publish adapter contract：[`contracts/v1.0-real-publish-adapter-guarded-contract.md`](contracts/v1.0-real-publish-adapter-guarded-contract.md)。
- 新增 guarded real publish adapter test matrix：[`testing/v1.0-real-publish-adapter-guarded-test-matrix.md`](testing/v1.0-real-publish-adapter-guarded-test-matrix.md)。
- 新增 metadata-only `publish_attempts` schema，记录 publish intent、review item、provider、source type、guard status、external call status、安全状态说明和本地状态变更原因；不保存 provider response、upload response、publish response 或任何 credential material。
- 新增 guarded adapter internal service 和 local-only backend API，用于基于 confirmed publish intent 创建本地 guarded publish attempt；不触发 provider publish call。
- 新增 frontend guarded attempt UI，用于展示本地 attempt 状态，并明确这不是上传、真实发布或排期发布。
- 新增 confirmed publish intent required、review/media/metadata/credential reference preflight、duplicate attempt prevention、real provider disabled、kill switch active、platform preconditions missing、sandbox fallback forbidden、no external service、no upload、no real publish 和 no metrics tests。
...
- 不发布视频。
- 不排期发布。
- 不新增真实外部 publish execution path 默认启用。
- 不新增 OAuth start route。
- 不新增 OAuth callback route。
- 不创建 OAuth URL。
- 不实现真实 OAuth。
- 不实现真实 token exchange。
- 不保存 authorization code、access token、refresh token、token、secret、credential、cookie、session、API key、bearer、raw OAuth state、raw request、raw response、provider response、upload response、publish response、external response 或 Douyin response。
- 不新增真实 token storage。
- 不新增真实 credential storage。
- 不新增 frontend OAuth UI。
- 不抓取真实指标。
...
- 新增 v1.0 publish status reconciliation ADR：[`decisions/0053-v1.0-publish-status-reconciliation.md`](decisions/0053-v1.0-publish-status-reconciliation.md)。
- 新增 publish status reconciliation contract：[`contracts/v1.0-publish-status-reconciliation-contract.md`](contracts/v1.0-publish-status-reconciliation-contract.md)。
- 新增 publish status reconciliation test matrix：[`testing/v1.0-publish-status-reconciliation-test-matrix.md`](testing/v1.0-publish-status-reconciliation-test-matrix.md)。
- 新增 metadata-only `publish_status_reconciliations` schema，记录 publish attempt、publish intent、review item、provider、source type、reconciliation status、local publish status、external query status、安全状态说明和本地状态变更原因；不保存 provider response、upload response、publish response、status response、external response、Douyin response、metrics response 或任何 credential material。
- 新增 metadata-only `publish_status_snapshots` schema，记录 publish attempt、reconciliation id、provider、source type、local publish status、status observed time、status source 和安全状态说明；不保存 raw provider / status / external response。
- 新增 internal reconciliation service 和 local-only backend API，用于基于 existing Batch 7 publish attempt 创建本地安全状态对账；不触发 provider status query。
- 新增 frontend Local Status Reconciliation / Publish Status Snapshot UI，用于展示本地状态对账和 snapshot，并明确这不是来自真实 Douyin 的状态。
- 新增 publish attempt required、duplicate reconciliation prevention、stale status ignored、malformed fixture rejected、real provider disabled、kill switch active、platform preconditions missing、sandbox fallback forbidden、no external service、no real status query、no upload、no real publish 和 no metrics tests。
...
- 不发布视频。
- 不排期发布。
- 不新增真实外部 status query path 默认启用。
- 不新增 OAuth start route。
- 不新增 OAuth callback route。
- 不创建 OAuth URL。
- 不实现真实 OAuth。
- 不实现真实 token exchange。
- 不保存 authorization code、access token、refresh token、token、secret、credential、cookie、session、API key、bearer、raw OAuth state、raw request、raw response、provider response、upload response、publish response、status response、external response、Douyin response 或 metrics response。
- 不新增真实 token storage。
- 不新增真实 credential storage。
- 不新增 frontend OAuth UI。
- 不启用 `douyin_real`。
...
- 新增 v1.0 user test readiness / release candidate ADR：[`decisions/0055-v1.0-user-test-readiness-release-candidate.md`](decisions/0055-v1.0-user-test-readiness-release-candidate.md)。
- 新增 RC checklist：[`releases/v1.0-user-test-rc-checklist.md`](releases/v1.0-user-test-rc-checklist.md)。
- 新增 release notes draft：[`releases/v1.0-user-test-release-notes-draft.md`](releases/v1.0-user-test-release-notes-draft.md)。
- 新增 PR description draft：[`releases/v1.0-pr-description-draft.md`](releases/v1.0-pr-description-draft.md)。
- 新增 user test guide：[`testing/v1.0-user-test-guide.md`](testing/v1.0-user-test-guide.md)。
- 新增 rollback / disablement runbook：[`operations/v1.0-user-test-rollback-disablement.md`](operations/v1.0-user-test-rollback-disablement.md)。
- 新增 RC test matrix：[`testing/v1.0-user-test-rc-test-matrix.md`](testing/v1.0-user-test-rc-test-matrix.md)。
- 可新增本地验证脚本，用于扫描测试、构建、敏感字段、禁用文案、schema forbidden columns 和外部能力边界；脚本不得调用外部服务、读取真实密钥或修改 runtime DB。

v1.0 Batch 10 明确不是：

- 不创建正式 v1.0 tag。
- 不创建或修改 `v1.0.0` tag。
- 不新增真实 Douyin provider runtime。
- 不调用 Douyin API 或业务外部服务。
- 不查询真实 Douyin metrics。
- 不查询真实 Douyin 发布状态。
- 不上传视频。
- 不发布视频。
- 不排期发布。
- 不新增真实外部 metrics query path。
- 不新增真实外部 status query path。
- 不新增 OAuth start route。
- 不新增 OAuth callback route。
- 不创建 OAuth URL。
- 不实现真实 OAuth。
...
- 不允许 `douyin_real` fallback 到 `douyin_sandbox`。
- 不声明 v1.0、v1.5 或 v2.0 已完成。
- 不声明 production、commercial 或 SaaS readiness。

范围（v1.0 目标，不代表当前 Batch 10 已实现）：
...
- v1.0 基于 v0.9.0 的 Douyin Provider POC / Sandbox Integration，把已验证的 sandbox provider contract、source separation、OAuth boundary、token lifecycle boundary、connection status boundary 和 metrics fallback 边界整理为用户测试规划；真实 OAuth、token、publish 和 metrics 实现仍需后续单独批次。
