...
本文档面向 v0.5 Batch 2 backend domain foundation 本地开发状态，说明如何在 Windows 11 和 PowerShell 下启动本地 backend、frontend，并验证内容项目、素材导入、ContentPlan / GenerationSchedule / Manual GenerationRun backend foundation 与 frontend UI foundation、Review Draft backend foundation 与 frontend UI foundation、PublishIntent / PublicationRecord backend foundation、Topic Candidate、Script Draft、Storyboard、fake render job、fake subtitle draft 和 fake preview manifest metadata 工作流。v0.5 当前只新增发布意图与发布记录的 backend-only 领域基础，不接真实 Douyin API，不实现 OAuth，不保存 token / secret / API key，不上传、不发布、不排期、不自动发布。
...
- PublishIntent create / list / read / cancel，以及必须基于同项目 approved Review Draft、archived project 只读、cross-project 访问 404、PublicationRecord list 为空和无真实发布 / 上传 / OAuth / Provider 副作用边界。
...
v0.5 Batch 2 新增了 backend-only 的 PublishIntent / PublicationRecord 领域基础。PublishIntent 只能基于同一项目内已 `approved` 的 Review Draft 显式创建；Review Draft approved 本身不会自动创建 PublishIntent，也不会触发发布、上传或排期发布。PublicationRecord 当前只提供数据表和按 PublishIntent 查询的读取接口；本批不提供 confirm / publish API，不接真实 PublisherProvider。
...
Invoke-RestMethod -Method Post "http://127.0.0.1:8000/api/projects/$($project.id)/publish-intents/$($intent.id)/cancel"
```

如果 Review Draft 不存在、属于其他项目或不是 `approved`，创建 PublishIntent 会返回错误；如果项目已归档，create / cancel 会返回 `409`，但仍允许读取已有 PublishIntent 和 PublicationRecord list。创建 PublishIntent 只写入 `publish_intents`，不会创建 `publication_records`，不会调用真实 Douyin API，不实现 OAuth，不保存 token、secret 或 API key，不上传、不发布、不排期、不自动发布，也不会创建媒体文件。
...
- 未实现真实抖音或其他平台发布；当前只有 PublishIntent / PublicationRecord backend domain foundation。
