...
v0.6.0 之后的架构路线必须从 fake/local metrics workflow 渐进进入抖音用户测试，而不是一次性跳到真实平台生产能力。

- v0.7 不改变 Provider 架构，只基于现有 `PublicationMetricSnapshot` 和 fake/local metrics source 做 metrics review summary，将指标转化为人工复盘参考。
