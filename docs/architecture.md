...
- v1.0 当前进入 Batch 2 OAuth state storage / anti-replay foundation 阶段，用于在 Batch 1 callback contract 之后建立 internal-only digest-only state foundation；真实 OAuth runtime、OAuth URL、OAuth start route、OAuth callback route、token exchange、credential storage、real provider API、publish workflow 和 metrics read 尚未实现。
...
### v1.0 Douyin OAuth Boundary, Callback Contract, And State Foundation

v1.0 Batch 0 introduced Douyin Integration User Test Release planning. v1.0 Batch 1 defines the future OAuth boundary and Douyin callback contract in ADR 0046 and [`contracts/v1.0-douyin-oauth-callback-contract.md`](contracts/v1.0-douyin-oauth-callback-contract.md). v1.0 Batch 2 adds ADR 0047 and [`contracts/v1.0-douyin-oauth-state-storage-contract.md`](contracts/v1.0-douyin-oauth-state-storage-contract.md) for the internal OAuth state storage / anti-replay foundation. Batch 2 does not add frontend UI, real Provider behavior, OAuth implementation, OAuth URLs, OAuth start routes, OAuth callback routes, token exchange, token storage, credential storage, real Douyin API calls, real metrics fetching, upload, publish, scheduling, or external service calls.
