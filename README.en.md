...
- Explicitly create a `PublishIntent` from an approved `Review Draft` in the same project, list/read publish intents, and cancel pending publish intents.
- Confirm a pending `PublishIntent`, mark it as `confirmed`, and create one local `not_started` `PublicationRecord` placeholder; this does not execute real platform actions.
- Execute a local fake publish for a confirmed PublishIntent, moving the matching `PublicationRecord` from `not_started` to `succeeded` with a stable fake external publication id; this does not mean a real platform publish succeeded.
