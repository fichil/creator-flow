...
The current v1.0 work line is based on the v0.9.0 release commit `0f5263452b65077f2c70c82e506944dd46e60e96`. v1.0 has moved to Batch 4 credential reference / encrypted storage design work after the Batch 3 token exchange boundary. Real OAuth, real token exchange, real token storage, real credential storage, real provider behavior, publish workflow, and metrics read implementation have not started. The repository still has no real Douyin integration, no OAuth implementation, no token storage, no real metrics fetching, no upload / publish / scheduling, and no external service calls.

v1.0 Batch 4 only adds an internal credential reference / credential storage boundary service, metadata-only reference tests, raw material rejection, encryption-provider-unavailable guard, real provider disabled behavior, sandbox fallback prevention, and matching documentation. Batch 4 does not create OAuth URLs, add OAuth start routes, add OAuth callback routes, perform real token exchange, store real tokens / secrets / credentials / authorization codes / raw OAuth state / OAuth state values, add encrypted payload storage, add frontend OAuth UI, or call Douyin APIs or business external services. Later real capabilities must wait for platform permissions, app review, user authorization, reliable encryption and key-management design, separate ADRs, tests, and security scans. v1.0 is not a production commercial release, not v1.5 Minimum Production Release, and not v2.0 Multi-Tenant SaaS Commercial Release.

## Roadmap To Commercial Release

The current released version is `v0.9.0 - Douyin Provider POC / Sandbox Integration`. v1.0 has entered Batch 4 credential reference / encrypted storage design work for the Douyin Integration User Test Release, but real OAuth runtime, real token exchange, and real credential storage have not started. The roadmap moves from v1.0 small-scope user testing to a v1.5 target Minimum Production Release for controlled direct-customer commercial use, and finally to a v2.0 target Multi-Tenant SaaS Commercial Release for customer-of-customer SaaS commercialization:
...
- v1.0 Douyin Integration User Test Release: currently in Batch 4 credential reference / encrypted storage design work, adding a metadata-only credential reference boundary, raw credential material rejection, encryption-provider-unavailable guard, real provider disabled behavior, sandbox fallback prevention, and no token material returned / persisted coverage after the Batch 3 token exchange boundary. The target remains validating whether real Douyin authorization, publishing, status tracking, and minimum metrics read are feasible; this batch does not implement real OAuth runtime, real token exchange, real token storage, or real credential storage. v1.0 is not a production commercial release and does not provide SLA, broad commercial use, or multi-tenant SaaS.
