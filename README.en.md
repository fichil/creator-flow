...
`v0.5 Human-Confirmed Publishing Workflow - Release Candidate`

This repository has completed the v0.1 local runnable skeleton, the v0.2 AI Planning Workflow, the v0.3 fake rendering/subtitle/preview workflow, the v0.4 local fake/manual Scheduled Draft Generation workflow, and v0.5 Batch 1 through Batch 6: the publishing boundary, PublishIntent / PublicationRecord backend foundation, PublishIntent confirm workflow, `FakePublisherProvider` local execution, the project detail Publishing / Fake Publishing workflow UI, Release Candidate stabilization / checklist, and final release validation documentation closure. The current path supports the local deterministic `FakeLLMProvider` workflow for Topic Candidates, Script Drafts, Storyboards, fake render jobs, fake subtitle drafts, and fake preview manifest metadata; v0.4 also supports project-level `ContentPlan`, `GenerationSchedule` configuration, fake manual `GenerationRun` records, and `Review Draft` placeholders created from manual runs. v0.5 currently supports a local fake/manual publishing workflow: a publish intent can only be explicitly created from an approved Review Draft in the same project, then read, cancelled, or confirmed; confirm only creates a local `not_started` `PublicationRecord` placeholder; the project detail page can create PublishIntents, confirm them, show PublicationRecords, and run local Fake Publish; fake publish only moves the local record to `succeeded`.
