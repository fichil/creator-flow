...
- v0.9 Douyin Provider POC / Sandbox Integration: Batch 0 completed planning, ADR, and checklist only. Batch 1 adds a backend-only Douyin Provider Adapter Skeleton for `douyin_sandbox` and `douyin_real`. Batch 2 completes sandbox-only simulated operations for `douyin_sandbox`, returning deterministic dry-run results and stable fake ids such as `sandbox_video_001` and `sandbox_publish_001`, while `douyin_real` remains blocked / not implemented. Batch 3 completes backend-only provider registry / factory routing so exact whitelisted provider ids can resolve to descriptors and adapters; `douyin_sandbox` routes to sandbox simulation, `douyin_real` remains blocked, and unknown providers do not fallback. Batch 3 is not real provider integration, not real OAuth, not real OAuth URL generation, not a real sandbox callback, and not real metrics read; it has no real Douyin API calls, no OAuth implementation, no OAuth callback route, no token exchange, no token storage, no real metrics fetching, no upload / publish / scheduling, no frontend UI, and no external service calls. Any later real Douyin API call requires a separate ADR, tests, and security scan.
