...
The current v1.0 work line is based on the v0.9.0 release commit `0f5263452b65077f2c70c82e506944dd46e60e96`. v1.0 has moved to Batch 3 token exchange boundary / fake-gated integration work after the Batch 2 state foundation. Real OAuth, real token exchange, credential storage, real provider behavior, publish workflow, and metrics read implementation have not started. The repository still has no real Douyin integration, no OAuth implementation, no token storage, no real metrics fetching, no upload / publish / scheduling, and no external service calls.

v1.0 Batch 3 only adds an internal token exchange boundary service, a fake-gated exchange simulator, state dependency / replay / expiry / provider isolation tests, and matching documentation. Batch 3 does not create OAuth URLs, add OAuth start routes, add OAuth callback routes, perform real token exchange, store tokens / secrets / credentials / authorization codes / raw OAuth state / OAuth state values, add frontend OAuth UI, or call Douyin APIs or business external services. Later real capabilities must wait for platform permissions, app review, user authorization, separate ADRs, tests, and security scans. v1.0 is not a production commercial release, not v1.5 Minimum Production Release, and not v2.0 Multi-Tenant SaaS Commercial Release.

## Roadmap To Commercial Release

The current released version is `v0.9.0 - Douyin Provider POC / Sandbox Integration`. v1.0 has entered Batch 3 token exchange boundary / fake-gated integration work for the Douyin Integration User Test Release, but real OAuth runtime and real token exchange have not started. The roadmap moves from v1.0 small-scope user testing to a v1.5 target Minimum Production Release for controlled direct-customer commercial use, and finally to a v2.0 target Multi-Tenant SaaS Commercial Release for customer-of-customer SaaS commercialization:
...
- v1.0 Douyin Integration User Test Release: currently in Batch 3 token exchange boundary / fake-gated integration work, adding a metadata-only fake-gated exchange boundary, real provider disabled behavior, sandbox fallback prevention, and no token material returned / persisted coverage after the Batch 2 state foundation. The target remains validating whether real Douyin authorization, publishing, status tracking, and minimum metrics read are feasible; this batch does not implement real OAuth runtime or real token exchange. v1.0 is not a production commercial release and does not provide SLA, broad commercial use, or multi-tenant SaaS.
