...
This repository has completed the v0.1 local runnable skeleton, the v0.2 AI Planning Workflow, the v0.3 fake rendering/subtitle/preview workflow, the v0.4 local fake/manual Scheduled Draft Generation workflow, and the v0.5 Human-Confirmed Publishing Workflow with the v0.5.0 release. v0.5 currently supports a local fake/manual publishing workflow: a publish intent can only be explicitly created from an approved Review Draft in the same project, then read, cancelled, or confirmed; confirm only creates a local `not_started` `PublicationRecord` placeholder; the project detail page can create PublishIntents, confirm them, show PublicationRecords, and run local Fake Publish; fake publish only moves the local record to `succeeded`. v0.6 has completed the Metrics Feedback Loop documentation foundation, the backend-only fake metrics foundation, the Fake Metrics Frontend UI foundation, and the Metrics Workflow Stabilization / RC checklist: the project detail page can show fake/local metrics snapshots for a `PublicationRecord` and manually generate a new fake metrics snapshot; it is currently only a local fake/manual metrics workflow RC candidate.

Local development instructions are available in [`docs/development.md`](docs/development.md), the v0.5 release candidate checklist is available in [`docs/releases/v0.5-rc-checklist.md`](docs/releases/v0.5-rc-checklist.md), and the v0.6 metrics RC checklist is available in [`docs/checklists/v0.6-metrics-feedback-loop-rc.md`](docs/checklists/v0.6-metrics-feedback-loop-rc.md).

Real OpenAI, Claude, Gemini, or other LLM integrations are still not implemented. The app does not store API keys, secrets, or tokens and does not call real AI services. v0.4 is still a local fake/manual workflow: scheduled `GenerationRun`, Scheduler / Trigger Engine, a complete `Review Queue`, real MP4 rendering, real video playback, FFmpeg, TTS, real subtitle files, real audio, production deployment, and user accounts are still not implemented. Review Drafts remain placeholders; approve / reject only changes review status and does not publish, upload, render, or generate media. The v0.5 publishing flow is still only a local fake workflow: confirm means the user has confirmed moving into publishing execution preparation, and Fake Publish succeeded only means local fake execution succeeded; it does not connect to the Douyin API, implement OAuth, store credentials, upload, publish, schedule, auto-publish, or connect a real PublisherProvider. v0.6 metrics are also only a fake/local workflow; the app does not fetch real metrics, connect to the real Douyin API, implement OAuth, store tokens, synchronize metrics on a schedule, provide analytics recommendation capabilities, or include a real platform dashboard.
...
- On the project detail page, create deterministic fake metrics snapshots for a specific `PublicationRecord`, then list or read metrics snapshots for that record; `source` is explicitly `fake_local` and does not represent real platform performance.
