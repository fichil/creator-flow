...
- v0.9 Douyin Provider POC / Sandbox Integration: starts with Batch 0 planning, ADR, and checklist only. The first phase must prioritize sandbox/mock callback planning and provider status transition dry-runs, with no real Douyin integration in this batch, no OAuth implementation, no OAuth callback route, no token exchange, no token storage, no real metrics fetching, no upload / publish / scheduling, and no external service calls. Any later real Douyin API call requires a separate ADR, tests, and security scan.
