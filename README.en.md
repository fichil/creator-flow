...
- v1.0 Douyin Integration User Test Release: currently in Batch 5 real provider feature flag / kill switch work, adding internal-only real provider controls, kill switch precedence, missing / malformed flag safe blocking, platform preconditions blocking, `douyin_real` default disabled behavior, and sandbox fallback prevention after the Batch 4 credential storage boundary. The target remains validating whether real Douyin authorization, publishing, status tracking, and minimum metrics read are feasible; this batch does not enable the real provider or implement real OAuth runtime, real token exchange, real token storage, real credential storage, real publishing, or real metrics read. v1.0 is not a production commercial release and does not provide SLA, broad commercial use, or multi-tenant SaaS.
