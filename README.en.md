...
The v0.9 Batch 9 release / PR merge preparation ADR is available in [`docs/decisions/0044-v0.9-release-merge-preparation.md`](docs/decisions/0044-v0.9-release-merge-preparation.md), with the PR description draft in [`docs/releases/v0.9-pr-description-draft.md`](docs/releases/v0.9-pr-description-draft.md), release notes draft in [`docs/releases/v0.9-release-notes-draft.md`](docs/releases/v0.9-release-notes-draft.md), merge readiness checklist in [`docs/releases/v0.9-merge-readiness-checklist.md`](docs/releases/v0.9-merge-readiness-checklist.md), and tag readiness checklist in [`docs/releases/v0.9-tag-readiness-checklist.md`](docs/releases/v0.9-tag-readiness-checklist.md). These files are only for human PR review / merge decision / tag decision; they do not mean `main` has been merged, a tag has been created, or a GitHub Release has been created.
...
Current stable release: v0.8.0 - Provider & Credential Security Foundation. Current main still has no real Douyin integration, no OAuth implementation, no OAuth callback route, no token exchange, no token storage, no real metrics fetching, no upload / publish / scheduling, and no external service calls. v0.9 starts with Douyin Provider POC / Sandbox Integration planning, and v0.9 Batch 0 is planning / ADR / checklist only; it does not turn the v0.8 placeholders into a real provider. v0.9 Batch 1 adds a backend-only Douyin Provider Adapter Skeleton, v0.9 Batch 2 adds deterministic sandbox-only simulated operations for `douyin_sandbox` while `douyin_real` remains blocked / not implemented, v0.9 Batch 3 adds a backend-only Douyin Provider Registry / Factory Routing foundation for exact whitelisted provider ids, v0.9 Batch 4 adds a backend-only sandbox metrics / mock workflow POC, v0.9 Batch 5 is docs-only / planning-only roadmap alignment, v0.9 Batch 6 adds sandbox-only backend API contract / smoke endpoints, v0.9 Batch 7 adds a frontend sandbox POC panel, and v0.9 Batch 8 prepares the POC readiness finalization / release candidate package with an RC checklist, test matrix, validation script, and docs consistency updates. Batch 8 is not the final v0.9 release; the next step is human review / PR / merge / release decision. It is not production-ready, not commercial-ready, not real Douyin integration, not OAuth, not token storage, not real metrics fetching, and not real upload / publish / scheduling.

v0.9 Batch 9 only prepares the release / PR merge review package, including the PR description draft, release notes draft, merge readiness checklist, tag readiness checklist, and ADR 0044. Batch 9 is not the final v0.9 release, does not mean `main` has been merged, does not create a `v0.9.0` tag, and does not create a GitHub Release. The next step remains human PR review / merge decision / final tag decision.

## Roadmap To Commercial Release

The current stable release is `v0.8.0 - Provider & Credential Security Foundation`. v0.8.0 has completed the Provider & Credential Security Foundation release; the next stage is v0.9 Douyin Provider POC / Sandbox Integration. The roadmap then moves from v1.0 user testing to a v1.5 target Minimum Production Release for controlled direct-customer commercial use, and finally to a v2.0 target Multi-Tenant SaaS Commercial Release for customer-of-customer SaaS commercialization:
...
- v0.9 Douyin Provider POC / Sandbox Integration: Batch 0 completed planning, ADR, and checklist only. Batch 1 adds a backend-only Douyin Provider Adapter Skeleton. Batch 2 completes sandbox-only simulated operations for `douyin_sandbox`, while `douyin_real` remains blocked / not implemented. Batch 3 completes backend-only provider registry / factory routing. Batch 4 completes a backend-only sandbox metrics / mock workflow POC. Batch 5 only aligns roadmap and readiness documentation from v1.0 to v2.0. Batch 6 adds sandbox-only backend API contract / smoke endpoints for provider descriptors, sandbox mock connection, sandbox metrics preview, and sandbox publish dry-run. Batch 7 adds a frontend sandbox POC panel as a visual sandbox smoke surface. Batch 8 adds the POC readiness finalization / release candidate package for human review / PR / merge / release decision. Batch 9 adds the release / PR merge preparation package for human PR review / merge decision / tag decision. v0.9 is not commercial, not production, not real provider integration, not real OAuth, not real OAuth URL generation, not real metrics read, and not real publish readiness; it has no real Douyin API calls, no OAuth implementation, no OAuth callback route, no token exchange, no token storage, no real metrics fetching, no upload / publish / scheduling, and no external service calls. Batch 9 does not declare v0.9 POC complete, does not automatically merge, does not create a tag, and does not create a GitHub Release.
- v1.0 Douyin Integration User Test Release: a future small-scope user test target for validating real Douyin authorization, publishing, status tracking, and minimum metrics read. It is not a production commercial release and does not provide SLA, broad commercial use, or multi-tenant SaaS.
