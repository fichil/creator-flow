...
- v0.9 Douyin Provider POC / Sandbox Integration: Batch 0 completed planning, ADR, and checklist only. Batch 1 adds a backend-only Douyin Provider Adapter Skeleton. Batch 2 completes sandbox-only simulated operations for `douyin_sandbox`, while `douyin_real` remains blocked / not implemented. Batch 3 completes backend-only provider registry / factory routing. Batch 4 completes a backend-only sandbox metrics / mock workflow POC. Batch 5 only aligns roadmap and readiness documentation from v1.0 to v2.0. Batch 6 adds sandbox-only backend API contract / smoke endpoints for provider descriptors, sandbox mock connection, sandbox metrics preview, and sandbox publish dry-run. Batch 7 adds a frontend sandbox POC panel as a visual sandbox smoke surface. Batch 8 adds the POC readiness finalization / release candidate package for human review / PR / merge / release decision. v0.9 is not commercial, not production, not real provider integration, not real OAuth, not real OAuth URL generation, not real metrics read, and not real publish readiness; it has no real Douyin API calls, no OAuth implementation, no OAuth callback route, no token exchange, no token storage, no real metrics fetching, no upload / publish / scheduling, and no external service calls. Batch 8 does not declare v0.9 POC complete and does not automatically release.
