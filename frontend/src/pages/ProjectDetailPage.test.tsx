...
const publicationRecordSecondSucceeded: PublicationRecord = {
  ...publicationRecordSucceeded,
  id: 1802,
...
const metricSnapshotWithSparseValues: PublicationMetricSnapshot = {
  ...metricSnapshotOne,
  id: 1902,
...
describe("ProjectDetailPage publishing workflow", () => {
  ...
  it("shows multiple metrics snapshots with readable fallback values", async () => {
    await renderProject({
      publicationMetrics: { 1801: [metricSnapshotOne, metricSnapshotWithSparseValues] },
      publicationRecords: { 1702: [publicationRecordSucceeded] },
      publishIntents: [publishIntentConfirmed],
      reviewDrafts: [reviewDraftApproved],
    });
    ...
    const recordCard = screen.getByLabelText("PublicationRecord 1801");
    const firstSnapshot = within(recordCard).getByLabelText("MetricSnapshot 1901");
    const sparseSnapshot = within(recordCard).getByLabelText("MetricSnapshot 1902");

    expect(firstSnapshot).toBeTruthy();
    expect(sparseSnapshot).toBeTruthy();
    expect(within(firstSnapshot).getByText("12.4s")).toBeTruthy();
    expect(within(firstSnapshot).getByText("72%")).toBeTruthy();
    expect(within(sparseSnapshot).getAllByText("Not provided").length).toBeGreaterThanOrEqual(3);
    expect(within(sparseSnapshot).getAllByText("Fake/local metrics").length).toBeGreaterThan(0);
    expect(within(sparseSnapshot).getByText("Not real platform performance.")).toBeTruthy();
  });
  ...
  it("refreshes only the targeted PublicationRecord metrics after fake generation", async () => {
    const server = await renderProject({
      publicationMetrics: {
        1802: [{ ...metricSnapshotOne, id: 1903, publication_record_id: 1802 }],
      },
    ...
    await waitFor(() =>
      expect(server.calls.filter((call) => call === "GET /api/projects/1/publication-records/1802/metrics")).toHaveLength(
        1,
      ),
    );
    fireEvent.click(within(firstRecordCard).getByRole("button", { name: "Generate fake metrics" }));

    expect(await within(firstRecordCard).findByLabelText("MetricSnapshot 1999")).toBeTruthy();
    const unchangedSecondRecordCard = screen.getByLabelText("PublicationRecord 1802");
    expect(within(unchangedSecondRecordCard).getByLabelText("MetricSnapshot 1903")).toBeTruthy();
    expect(within(unchangedSecondRecordCard).queryByLabelText("MetricSnapshot 1999")).toBeNull();
    expect(server.calls.filter((call) => call === "GET /api/projects/1/publication-records/1801/metrics")).toHaveLength(
      2,
    );
    expect(server.calls.filter((call) => call === "GET /api/projects/1/publication-records/1802/metrics")).toHaveLength(
