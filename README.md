...
当前 v1.0 工作线基于 v0.9.0 release commit `0f5263452b65077f2c70c82e506944dd46e60e96`。v1.0 已从 Batch 0 planning 推进到 Batch 1 OAuth boundary / callback contract 阶段，用于定义未来真实 Douyin OAuth authorization start 和 callback 的 contract 边界；本阶段仍未开始真实 OAuth、token lifecycle、credential storage、real provider、publish workflow 或 metrics read 实现。当前仓库仍不接真实 Douyin，不实现 OAuth，不保存 token，不抓取真实指标，不上传、不发布、不排期发布，也不适合作为生产级真实平台集成使用。

v1.0 Batch 1 只定义 OAuth 边界、未来 callback contract、redaction 规则、provider source separation、dependency gate、feature flag / kill switch 前置约束和后续 contract test 计划。Batch 1 不创建 OAuth URL，不新增 OAuth callback route，不新增 OAuth state storage，不做 token exchange，不保存 token / secret / credential / authorization code / OAuth state，不新增数据库表，不新增 backend API，不新增 frontend OAuth UI，不调用 Douyin API 或外部服务。后续真实能力必须等平台权限、应用审核、用户授权和单独 ADR / tests / security scan 完成后才能实现。v1.0 不是生产商用版本，不等于 v1.5 Minimum Production Release，也不等于 v2.0 Multi-Tenant SaaS Commercial Release。

## 后续路线

当前已发布版本是 `v0.9.0 - Douyin Provider POC / Sandbox Integration`。v1.0 已进入 Douyin Integration User Test Release 的 Batch 1 OAuth boundary / callback contract 阶段，但尚未开始真实实现。后续路线从 v1.0 小范围用户测试逐步扩展到 v1.5 直接客户受控商用目标，再到 v2.0 面向客户的客户 SaaS 商用目标：
...
- v1.0 Douyin Integration User Test Release：当前进入 Batch 1 OAuth boundary / callback contract，用于定义未来真实 Douyin OAuth authorization start、callback、error callback、cancelled authorization、malformed callback、replayed callback、expired callback 和 unsupported provider 的 contract 边界。目标仍是验证真实 Douyin 授权、发布、状态查询和最小指标回流是否可行；本批不实现真实能力。v1.0 不是生产商用版本，也不承诺 SLA、批量商用或多租户 SaaS。
