...
creator-flow 是一个可开源的 AI 短视频内容流水线，帮助你将显式导入的想法、聊天摘要、文本、图片、截图和链接转化为待审核的短视频草稿。

## 当前状态

`v0.8.0 - Provider & Credential Security Foundation`

当前仓库已完成 v0.1 本地可运行骨架、v0.2 AI Planning Workflow、v0.3 fake rendering/subtitle/preview workflow、v0.4 local fake/manual Scheduled Draft Generation workflow、v0.5 Human-Confirmed Publishing Workflow、v0.6.0 local fake/manual metrics feedback workflow、v0.7.0 Metrics Review Summary local fake/manual workflow，以及 v0.8.0 Provider & Credential Security Foundation。v0.8.0 完成 Provider Registry & Capability Metadata、Provider Connection State、Provider Credential Reference & Secret Redaction、Provider Security Audit Event & Redacted Audit Log、Provider OAuth State & Callback Boundary、Provider Token Lifecycle Boundary、Provider Integration Readiness Summary、对应 frontend read-only UI panels，以及 v0.8 RC checklist / closure audit。
...
v0.9 Batch 9 的 release / PR merge preparation ADR 见 [`docs/decisions/0044-v0.9-release-merge-preparation.md`](docs/decisions/0044-v0.9-release-merge-preparation.md)，PR 描述草案见 [`docs/releases/v0.9-pr-description-draft.md`](docs/releases/v0.9-pr-description-draft.md)，release notes 草案见 [`docs/releases/v0.9-release-notes-draft.md`](docs/releases/v0.9-release-notes-draft.md)，merge readiness checklist 见 [`docs/releases/v0.9-merge-readiness-checklist.md`](docs/releases/v0.9-merge-readiness-checklist.md)，tag readiness checklist 见 [`docs/releases/v0.9-tag-readiness-checklist.md`](docs/releases/v0.9-tag-readiness-checklist.md)。这些文件仅用于 human PR review / merge decision / tag decision，不表示已经 merge main、创建 tag 或创建 GitHub Release。

v0.8.0 仍不接真实 Douyin，不实现 OAuth，不新增真实 OAuth callback route，不新增 OAuth state storage，不新增 token exchange，不生成真实 provider authorization URL，不保存 token、secret、API key、credential、authorization code 或 OAuth state，不抓取真实指标，不上传、不发布、不排期发布，也不调用外部服务。v0.8.0 是安全边界 foundation release，不是 production-ready real Douyin integration，也不是生产部署版本。

当前 main 仍不接真实 Douyin，不实现 OAuth，不保存 token，不抓取真实指标，不上传、不发布、不排期发布，也不适合作为生产级真实平台集成使用。v0.9 是 Douyin Provider POC / Sandbox Integration 阶段，v0.9 Batch 0 仅做 planning、ADR 和 checklist，不会直接把 v0.8 placeholder 变成真实 provider；v0.9 Batch 1 新增 backend-only Douyin Provider Adapter Skeleton；v0.9 Batch 2 仅为 `douyin_sandbox` 增加确定性的 sandbox-only operation simulation，并保持 `douyin_real` blocked / not implemented；v0.9 Batch 3 新增 backend-only Douyin Provider Registry / Factory Routing foundation，可通过白名单 provider id 路由到 `douyin_sandbox` 或 `douyin_real`，unknown provider 不会 fallback；v0.9 Batch 4 新增 backend-only sandbox metrics / mock workflow POC；v0.9 Batch 5 仅完成 docs-only / planning-only 的 roadmap alignment；v0.9 Batch 6 新增 sandbox-only backend API contract / smoke endpoints；v0.9 Batch 7 新增 frontend Douyin Sandbox POC Panel，只展示 deterministic sandbox / simulated / dry-run provider descriptors、mock connection、metrics preview 和 publish dry-run result；v0.9 Batch 8 仅准备 POC readiness finalization / release candidate package，补齐 RC checklist、test matrix、validation script 和文档一致性收口。Batch 8 不等于 v0.9 final release，下一步仍是人工 review / PR / merge / release decision；本仓库不自动打 tag、不自动 merge main、不创建 GitHub Release，也不声明 v0.9 POC 已完成。

v0.9 Batch 9 仅准备 release / PR merge review package，新增 PR 描述草案、release notes 草案、merge readiness checklist、tag readiness checklist 和 ADR 0044。Batch 9 不等于 v0.9 final release，不表示已经 merge main，不创建 `v0.9.0` tag，不创建 GitHub Release；下一步仍是 human PR review / merge decision / final tag decision。

## 后续路线

当前稳定版本是 `v0.8.0 - Provider & Credential Security Foundation`。v0.8.0 已完成 Provider & Credential Security Foundation release；下一阶段是 v0.9 Douyin Provider POC / Sandbox Integration，后续路线从 v1.0 用户测试逐步扩展到 v1.5 直接客户受控商用目标，再到 v2.0 面向客户的客户 SaaS 商用目标：
...
- v0.9 Douyin Provider POC / Sandbox Integration：Batch 0 已完成 planning、ADR 和 checklist；Batch 1 新增 backend-only Douyin Provider Adapter Skeleton；Batch 2 已完成 `douyin_sandbox` 的 sandbox-only simulated operations，返回 deterministic dry-run result 和稳定 fake id，例如 `sandbox_video_001`、`sandbox_publish_001`，同时 `douyin_real` 继续 blocked / not implemented；Batch 3 已完成 backend-only provider registry / factory routing foundation；Batch 4 已完成 backend-only sandbox metrics / mock workflow POC；Batch 5 仅完成 v1.0 到 v2.0 的 roadmap / readiness documentation alignment；Batch 6 新增 sandbox-only backend API contract / smoke endpoints，暴露 provider descriptor list / lookup、sandbox mock connection、sandbox metrics preview 和 sandbox publish dry-run 的可测试 API 边界；Batch 7 新增 frontend sandbox POC panel，作为 visual sandbox smoke surface；Batch 8 新增 POC readiness finalization / release candidate package，用于 human review / PR / merge / release decision。v0.9 仍不是商用版本，不是真实 provider integration，不调用真实 Douyin API，不实现 OAuth，不创建 OAuth URL，不新增 OAuth callback route，不交换或保存 token，不抓取真实指标，不上传、不发布、不排期发布；Batch 8 不声明 v0.9 POC 已完成，也不自动发布。
- v0.9 Batch 9 Release Branch / PR Merge Preparation：仅准备 human PR review / merge decision / tag decision 材料，包括 PR 描述草案、release notes 草案、merge readiness checklist、tag readiness checklist 和 ADR 0044；不自动 merge main，不创建 `v0.9.0` tag，不创建 GitHub Release，也不把 release preparation 写成最终发布。
- v1.0 Douyin Integration User Test Release：未来才进入面向小范围用户测试的抖音接入版本，目标是验证真实 Douyin 授权、发布、状态查询和最小指标回流是否可行；它仍不是大规模生产商用版本，也不承诺 SLA、批量商用或多租户 SaaS。
