...
当前 v1.0 工作线基于 v0.9.0 release commit `0f5263452b65077f2c70c82e506944dd46e60e96`。v1.0 已推进到 Batch 4 credential reference / encrypted storage design 阶段，用于在 Batch 3 token exchange boundary 之后建立 internal-only credential storage boundary；本阶段仍未开始真实 OAuth、真实 token exchange、真实 token storage、真实 credential storage、real provider、publish workflow 或 metrics read 实现。当前仓库仍不接真实 Douyin，不实现 OAuth，不保存 token，不抓取真实指标，不上传、不发布、不排期发布，也不适合作为生产级真实平台集成使用。

v1.0 Batch 4 只新增 internal-only credential reference / credential storage boundary service、metadata-only reference 测试、raw material rejection、encryption provider unavailable guard、real provider disabled、sandbox fallback forbidden 和对应文档。Batch 4 不创建 OAuth URL，不新增 OAuth start route，不新增 OAuth callback route，不做真实 token exchange，不保存真实 token / secret / credential / authorization code / raw OAuth state / OAuth state value，不新增 encrypted payload storage，不新增 frontend OAuth UI，不调用 Douyin API 或业务外部服务。后续真实能力必须等平台权限、应用审核、用户授权、可靠加密与密钥管理设计、单独 ADR / tests / security scan 完成后才能实现。v1.0 不是生产商用版本，不等于 v1.5 Minimum Production Release，也不等于 v2.0 Multi-Tenant SaaS Commercial Release。

## 后续路线

当前已发布版本是 `v0.9.0 - Douyin Provider POC / Sandbox Integration`。v1.0 已进入 Douyin Integration User Test Release 的 Batch 4 credential reference / encrypted storage design 阶段，但尚未开始真实 OAuth runtime、真实 token exchange 或真实 credential storage。后续路线从 v1.0 小范围用户测试逐步扩展到 v1.5 直接客户受控商用目标，再到 v2.0 面向客户的客户 SaaS 商用目标：
...
- v1.0 Douyin Integration User Test Release：当前进入 Batch 4 credential reference / encrypted storage design，用于在 Batch 3 token exchange boundary 后建立 metadata-only credential reference、raw credential material rejection、encryption provider unavailable guard、real provider disabled、sandbox fallback forbidden 和 no token material returned / persisted 基础。目标仍是验证真实 Douyin 授权、发布、状态查询和最小指标回流是否可行；本批不实现真实 OAuth runtime、真实 token exchange、真实 token storage 或真实 credential storage。v1.0 不是生产商用版本，也不承诺 SLA、批量商用或多租户 SaaS。
