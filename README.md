...
- v0.8 Provider & Credential Security Foundation：Batch 1 已完成 Provider、Credential、OAuth、Secret 和 token lifecycle 的文档 / ADR 边界；Batch 2 已完成 backend-only Provider Registry & Capability Metadata foundation；Batch 3 已完成 frontend read-only Provider Registry UI foundation；Batch 4 已完成 backend-only Provider Connection State & Sensitive Storage Status foundation；Batch 5 已完成 frontend read-only Provider Connection State UI foundation；Batch 6 已完成 backend-only Provider Credential Reference & Secret Redaction foundation；Batch 7 已完成 frontend read-only Provider Credential Reference UI foundation；Batch 8 已完成 backend-only Provider Security Audit Event & Redacted Audit Log foundation；Batch 9 是 frontend read-only Provider Security Audit Event UI foundation。前端只展示 provider security audit event metadata、source type、implementation status、`event_type`、`event_status`、`event_severity`、`actor_type`、`redaction_status`、`safe_event_message`、`safe_metadata`、`boundary_notes` 和 `created_at`。本阶段不接真实 Douyin、不实现 OAuth、不新增 OAuth callback route、不保存 token、不保存 secret、不保存 API key、不保存 authorization code、不保存 OAuth client secret、不保存 credential material、不保存 raw request、raw response 或 raw payload、不新增真实 Credential storage、不新增 audit event 写入 UI、不提供 secret input、token viewer 或 credential 管理界面、不抓取真实指标、不上传、不发布、不排期发布，也不提供连接、授权、刷新、撤销或断开连接操作。
