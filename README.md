...
当前 main 仍不接真实 Douyin，不实现 OAuth，不保存 token，不抓取真实指标，不上传、不发布、不排期发布，也不适合作为生产级真实平台集成使用。v0.9 是 Douyin Provider POC / Sandbox Integration 阶段，但 v0.9 Batch 0 仅做 planning、ADR 和 checklist，不会直接把 v0.8 placeholder 变成真实 provider。
...
- v0.9 Douyin Provider POC / Sandbox Integration：先从 Batch 0 的 planning、ADR 和 checklist 开始；第一阶段必须优先 sandbox/mock callback 和 provider status transition dry-run，不会直接把 v0.8 placeholder 变成真实 provider。后续任何真实 Douyin API 调用、真实 OAuth、真实 token exchange / storage 或真实指标读取都必须单独 ADR、单独测试和安全扫描。
