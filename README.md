...
- v0.9 Douyin Provider POC / Sandbox Integration：Batch 0 已完成 planning、ADR 和 checklist；Batch 1 新增 backend-only Douyin Provider Adapter Skeleton；Batch 2 已完成 `douyin_sandbox` 的 sandbox-only simulated operations，返回 deterministic dry-run result 和稳定 fake id，例如 `sandbox_video_001`、`sandbox_publish_001`，同时 `douyin_real` 继续 blocked / not implemented；Batch 3 已完成 backend-only provider registry / factory routing foundation，可通过白名单 provider id 获取 descriptor 并创建对应 adapter，`douyin_sandbox` 路由到 sandbox simulation，`douyin_real` 被识别但仍 blocked / not implemented，unknown provider 不会 fallback；Batch 4 已完成 backend-only sandbox metrics / mock workflow POC，通过 factory / registry 路由 `douyin_sandbox` 并返回 deterministic mock account connection、sandbox metrics payload 和 dry-run publish result，`douyin_real` 仍 blocked / not implemented。Batch 4 不是真实 provider integration，不调用真实 Douyin API，不实现 OAuth，不创建 OAuth URL，不新增 OAuth callback route，不交换或保存 token，不抓取真实指标，不上传、不发布、不排期发布，也不新增前端 UI。后续第一阶段仍必须优先 sandbox/mock callback 和 provider status transition dry-run；任何真实 Douyin API 调用、真实 OAuth、真实 token exchange / storage 或真实指标读取都必须单独 ADR、单独测试和安全扫描。
